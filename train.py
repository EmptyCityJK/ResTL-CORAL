...
def one_epoch_distribution(net, data_loader, optimizer, loss, hyper, args, train='train'):
    ...
    for i_iter, sample_train in enumerate(data_loader):
        ...
        # print(f"x_anc shape: {x_anc.shape}") # ([128, 1, 62, 750])
        # print(f"x_pos shape: {x_pos.shape}") # ([128, 1, 62, 750])
        # print(f"x_neg shape: {x_neg.shape}") # ([128, 1, 62, 750])
        # print(f"x_anc_rest shape: {x_anc_rest.shape}") # ([128, 1, 62, 750])
        # print(f"x_pos_rest shape: {x_pos_rest.shape}") # ([128, 1, 62, 750])
        # print(f"x_neg_rest shape: {x_neg_rest.shape}") # ([128, 1, 62, 750])
        ...
        if train == 'train' or train == 'fine' or train == 'RS':
            ...
            if args.cdist != 0 and hyper[1] != 0:
                for i, (dm, ds) in enumerate(zip(dist_mu, dist_sigma)):
                    if i >= net.mu.shape[0]:  # 安全检查
                        continue
                    if net.mu.shape[1] == 1:
                        net.mu[i].data -= dm * args.cdist
                        net.sigma[i].data -= ds * args.cdist
                # for i in range(args.datanum):
                #     if net.mu.shape[1] == 1:
...
def generate_signal_from_RS(net, data_loader, loss, hyper, args, train='train'):
    net.eval()
    dataset_RS = [] # 原始的 resting-state EEG 样本（x_anc_rest）
    dataset_RS_update = [] # 通过优化从 RS 生成的伪 TS（task-state）信号，拟作 x_anc、x_pos、x_neg
    dataset_RS_label = [] # 对应的 one-hot label
    for i_iter, sample_train in enumerate(data_loader):
        RS_init_list = []
        RS_update = []
        RS_update_label = []

        # print(f"🌀 [Batch {i_iter}] Received input RS shape: {sample_train.shape}")  # [B, 1, 62, 750]
        # assert sample_train.dim() == 4 and sample_train.shape[1:3] == (1, args.chnum), \
        #     f"[!] Unexpected shape: {sample_train.shape}"
        ...
        for label in range(args.datanum):
            ...
            while run == 0:
                ...
                # if u_iter % 100 == 0:
                #     print(f"   🔁 label={label} iter={u_iter} loss={miniloss.item():.4f} | f_cls={f_cls.item():.4f} | clsloss={clsloss.item():.4f}")

                if u_iter == 300:
                    RS_init_list.append(RS.detach().cpu()) # 原始RS
                    RS_update.append(RS_init.data.detach().cpu()) # 生成 TS
                    RS_update_label.append(pseudo_label) # 对应标签
                    run = 1
                else:
                    u_iter += 1

        RS_init_ = torch.cat(RS_init_list, dim=0)
        # print(RS_init_.shape)
        RS_update_ = torch.cat(RS_update, dim=0)
        # print(RS_update_.shape)
        RS_update_label_ = torch.stack(RS_update_label, dim=0)
        # print(RS_update_label_.shape)

        # 🔍 Debug 确保尺寸正确
        assert RS_init_.shape == RS_update_.shape, f"[!] Shape mismatch: RS_init_ {RS_init_.shape} vs RS_update_ {RS_update_.shape}"
        assert RS_init_.shape[-1] == 750 and RS_init_.shape[1] == 1 and RS_init_.shape[2] == args.chnum, \
            f"[!] Unexpected RS shape: {RS_init_.shape}"
        assert RS_update_label_.shape[1] == args.datanum, f"[!] Label shape mismatch: {RS_update_label_.shape}"

        # print(f"✅ [Batch {i_iter}] RS_init: {RS_init_.shape}, RS_update: {RS_update_.shape}, Label: {RS_update_label_.shape}")
